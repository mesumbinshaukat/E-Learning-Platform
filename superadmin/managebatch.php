			...
			<div class="main-container container-fluid">
									...
									<table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
										...
										<tbody>
											...
											if (mysqli_num_rows($batch_query) > 0) {
												$i = 1;
												$unique_courses = array(); // Array to store unique course names
												...
												while ($batch_row = mysqli_fetch_assoc($batch_query)) {
													if ($batch_row["status"] === "Active") {
														...
														if (!in_array($course_name, $unique_courses)) {
															...
															echo "<td>" . $batch_row['batchtrainer_name'] . "</td>";
															echo "<td>" . $course_name . "</td>";

															// Count the occurrences of the current course name
															$count = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `batch` WHERE `batchcourse_name` = '$course_name' && `status` = 'Active'"));
															echo "<td>{$count}</td>";
                ...
                <div class="col-sm-6 col-md-15 mg-t-10 mg-sm-t-0">
                    ...
                    <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item" href="addbatchstudent.php?trainer_id=' . $batch_row['batchtrainer_id'] . '&course_id=' . $batch_row['batchcourse_id'] . '&batch_id=' . $batch_row['id'] . '">Add</a>
                        <a class="dropdown-item" href="removebatchstudent.php?trainer_id=' . $batch_row['batchtrainer_id'] . '&course_id=' . $batch_row['batchcourse_id'] . '&batch_id=' . $batch_row['id'] . '">Remove</a>
                        <a class="btn dropdown-item" href="connection_files/manage/batch_manage.php?id=' . $batch_row['id'] . '&delete=delete">Delete</a>
                        <a class="dropdown-item" href="batch_manage.php?id=' . $batch_row['id'] . '&status=complete">Complete</a>
                    </div>
                </div>
            </td>';

															// Add the course name to the unique courses array

			...
			<div class="main-container container-fluid">
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="right-content">
						<span class="main-content-title mg-b-0 mg-b-lg-1" style="color:#ff6700">Add Placement
							Registrations</span>
					</div>
								...
								<div class="table-responsive  export-table">
									<table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
										...
										<tbody>
											...
											<?php
											$select_reg_query = mysqli_query($conn, "SELECT job_id, MIN(applied_date) AS applied_date FROM `placement_applicants` GROUP BY job_id");

											if ($select_reg_query) {
												$i = 1;
												while ($row = mysqli_fetch_assoc($select_reg_query)) {
													$select_internship = mysqli_query($conn, "SELECT `id`, `job_role`, `vacancies`, `last_date_to_apply` FROM `placement` WHERE `id`='{$row["job_id"]}'");
													...
													if ($select_internship) {
														if (mysqli_num_rows($select_internship) > 0) {
															...
															echo "<td>" . $fetch_internship["job_role"] . "</td>";
															echo "<td>" . $fetch_internship["vacancies"] . "</td>";
															echo "<td>" . $fetch_internship["last_date_to_apply"] . "</td>";
															echo "<td><a href='selectstudentplacement.php?id=" . $fetch_internship["id"] . "&type=placement' class='btn btn-info'>Add Student</a></td>";
															echo "</tr>";
														}
													} else {
